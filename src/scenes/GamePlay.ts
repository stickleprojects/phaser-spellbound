...
export class GamePlay extends Phaser.Scene {
  ...
  async create() {
    ...
    this._levelConfig = new LevelConfig(
      this.cache,
      this.roomWidthInTiles,
      this.map.tileWidth,
      this.roomHeightInTiles,
      this.map.tileHeight
    );
    ...
    this.RoomNavigator = new RoomNavigator(
      this.input,
      this.horizontalRooms,
      this.verticalRooms,
      () => this.positionCameraAccordingToRoom()
    );

    this.map_alltiles = this.map.addTilesetImage("alltiles", "map_background")!;
    this.map_foregroundtiles = this.map.addTilesetImage(
      "foregroundtiles",
      "map_foreground"
    )!;
    ...
    this.LiftManager = await LiftManager.CreateAsync(
      this.Doors,
      this.input,
      this.sound
    );
    ...
    if (!pnl) {
      ...
      this._liftControlPanel = new LiftControlPanel(
        this,
        pnl.x!,
        pnl.y!,
        this.LiftManager
    ...
    const w =
      this.roomWidthInTiles * this.map.tileWidth * this.cameras.main.zoomX;
    const h =
      this.roomHeightInTiles * this.map.tileHeight * this.cameras.main.zoomY;
  ...
  createDoors(doorTileObjects: Phaser.Types.Tilemaps.TiledObject[]) {
    ...
    doorTileObjects.forEach((o) => {
      ...
      let sprite = this.physics.add.staticSprite(
        pixelX - objecthalfW,
        pixelY,
        "characters",
        frameName
  ...
  examine() {
    ...
    if (!r) {
      console.error(
        "Failed to get topmostdialog.data.dimensions, got undefined instead"
      );
      return;
    }
    ...
    const chooseWhat = new ShowWhatDialogParameters(
      this,
  ...
  getCommandList(): CommandItem[] {
    ...
    const commands = [
      ...
      i.HasItem("teleportkey")
        ? new CommandItem("Teleport", (_) => {
            this._dialogManager.closeTopmost();
  ...
  showDialog() {
    ...
    var dialogParameters = new CommandDialogParameters(
      this,
  ...
  showMessage(txt: string[] | string) {
    ...
    var dialogParameter = new MessageDialogParameters(
      this,
  ...
  showNearbyCharacterSelector(onselected: (itm: CharacterSprite) => void) {
    ...
    const menuItems = nearbyItems.map(
      (itm: CharacterSprite, _) =>
  ...
  private wireupEvents() {
    customEmitter.onCallLift(async (_) => {
      const playerFloor = this.LiftManager.GetClosestLiftDoor(
    ...
    this.objectLayer.forEach((o) => {
      ...
      if (!itemInfo) {
        ...
        if (itemInfo.stats?.glows) {
          const objectLight = this.lights.addLight(
            pixelX,
            pixelY,
            50,
            0xfcc603,
  ...
  createKnight(x: number, y: number, index: number) {
    ...
    const nearbySprite = this.physics.add.body(
      x,
      y,
      nearbyWidth,
      sprite.height
    ...
    this.physics.add.collider(
      ...
      this.solidLayer,
      undefined,
      this.ignoreInvisibleItemsCollider.bind(this)
    );
    this.physics.add.collider(
      sprite,
      this.DoorGroup,
    ...
    this._liftDoorKnightOverlap = this.physics.add.overlap(
      sprite,
      this.DoorGroup,
      undefined,
      this.liftDoorCollision,
      this
    );
  ...
  positionCameraAccordingToRoom() {
    ...
    if (!roomInfo) {
      ...
      this.camera.setBounds(
        roomInfo.WorldLocation.x,
        roomInfo.WorldLocation.y,
        roomInfo.WorldLocation.width,
        roomInfo.WorldLocation.height,
        false
      );

      const glowingItem = this.Player.getInventory().FindItem(
        (i) => i.stats?.glows == true
      );

      if (this.flags.FollowingPlayer) {
        if (roomInfo.stats.dark) {
          if (!glowingItem) {
            customEmitter.emitShowMessage([
  ...
  teleportPlayerToPad() {
    ...
    if (this.Player.getInventory().HasItem("teleportpad")) {
      console.error(
        "you are CARRYING the teleport pad! you cant teleport to it"
      );
      return;
    }
  ...
  pickupNearestItem() {
    ...
    if (nearbyItems.length == 0) {
      ...
      let result = this.Player.getInventory().AddItem(
        itemToPickup as IInventoryItem
      );
      if (result.ok) {
        // great
      } else {
