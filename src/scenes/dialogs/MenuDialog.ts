...
export class MenuDialog extends Dialog {
  ...
  protected calcWidthOfMenuItems(data: MenuDialogParameters): number {
    const maxChars = data.menuitems.reduce(
      (a: number, b: string) =>
        b.toString().length > a ? b.toString().length : a,
      0
    );
  ...
  protected calcHeightOfMenuItems(data: MenuDialogParameters): number {
    ...
    return (
      borderHeight +
      this._itemGapTop +
      itemCount * this._itemHeight +
      (itemCount - 1) * this._itemGapBetween
    );
  }
  ...
  addControls(data: MenuDialogParameters, innerRect: Rectangle) {
    ...
    if (data.allowSelection) {
      this._selectionIndicator = this.add.rectangle(
        0,
        0,
        innerRect.width,
        itemHeight
      ...
      this._selectionIndicator.setStrokeStyle(
        2,
        Phaser.Display.Color.GetColor(200, 200, 200)
      );
